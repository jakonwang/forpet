...
class FileUpload {
	...
	public function __construct($_file,$_maxsize) {
		...
		$this->today = $this->path.$this->linktotay;/*绝对路径目录*/
		$this->name = $_FILES[$_file]['name'];//文件名
		$this->tmp = $_FILES[$_file]['tmp_name'];//临时文件路径
		$this->checkError();
		$this->checkType();
		$this->checkPath();
		$this->moveUpload();
	...
	private function setNewName() {
		...
		$this->linkpath = UPLOAD_DIR.$this->linktotay.$_newname;
