...
<body>
	...
	<main id='addFriendsList'>
		<ul>
		<{if $usersList}>
			<{foreach from=$usersList key=key item=item}>
			<li class='friends'>
				<a href="#">
				<img src="<{$item->headimgurl}>" alt="" class='headimgurl'>
				<div>
					<h2><{$item->nickname}><img src='<{$smarty.const.ROOT_URL}>/images/my/<{$item->sex}>.png'/></h2>
					<p class='location'><{$item->province}><{$item->city}></p>
				</div>
				</a>
				<a href='friends.php?action=addFriends&uid=<{$item->id}>' class="add" title='添加为好友'>添加</a>
			</li>
			<{/foreach}>
		<{/if}>	
		</ul>
