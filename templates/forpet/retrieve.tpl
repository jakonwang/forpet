...
<body>
	...
	<main id='retrieve'>
		<ul>
		<{if $allRetrieve}>
			<{foreach from=$allRetrieve key=key item=item}>
			<li class='item'>
				<div class="thumb"><img src="<{$item->headimgurl}>" alt="<{$item->nickname}>"></div>
				<div class="title">
					<h2><a href='###'><{$item->nickname}></a><span class='type'><{$item->category_name}></span></h2>
					<p><{$item->pubdate}> 来自 <{$item->location}></p>
				</div>
				<div class="description"><{$item->description}>...</div>
				<div class="pic">
				<{if $item->imgsrc}>
				<a href="retrieve.php?action=detail&id=<{$item->id}>">
	...
	<!--E footer-->
	<script src='<{$smarty.const.ROOT_URL}>/js/retrieve.js'></script>
	<!--S 发布按钮-->
	<script src='<{$smarty.const.ROOT_URL}>/js/release.js'></script>
	<!--S ajax获取下一页-->
	<script src='<{$smarty.const.ROOT_URL}>/js/retrieveAjax.js'></script>
	<!--E ajax获取下一页-->
</body>
</html>